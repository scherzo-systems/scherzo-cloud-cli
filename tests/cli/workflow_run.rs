...
printf '{"type":"system","subtype":"init","cwd":"%s","session_id":"%s","model":"%s","permissionMode":"bypassPermissions","claude_code_version":"2.1.263"}\n' "$PWD" "$session" "$model"
...
fn claude_code_only_run_pins_the_validated_executable_without_pi_or_path_fallback() {
    let replacement =
        ClaudeCodeFixture::new("2.1.263 (Claude Code)", CLAUDE_CODE_COMPLETE_HELP, false);
...
fn local_admission_probes_only_the_harness_selected_by_the_workflow() {
    ...
    let claude = ClaudeCodeFixture::new("2.1.263 (Claude Code)", CLAUDE_CODE_COMPLETE_HELP, true);
