...
fn doctor_reports_exact_codex_identity_without_credentials_or_native_configuration() {
    ...
    let fixture = CodexFixture::new("0.154.23", true, true);
    ...
    assert_eq!(check["details"]["version"], "0.154.23");
    assert_eq!(check["details"]["profile"], "CodexAppServerV1");
    assert_eq!(check["details"]["supportedRange"], ">=0.147.0 <0.155.0");
    assert_eq!(check["details"]["qualificationVersion"], "0.154.0");
...
fn doctor_does_not_fall_back_after_the_first_path_candidate_fails() {
    ...
    let fallback = CodexFixture::new("0.154.23", true, true);
    let path =
