...
`2.1.280`; accepting the range does not claim exact-binary conformance for every release
...
`>=0.147.0 <0.155.0`, and require the maintained App Server schema capability, including
the ephemeral-thread contract. The exact repository qualification anchor is `0.154.0`;
