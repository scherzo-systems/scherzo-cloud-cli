...
capabilities, and the selected absolute path. Qualification remains pinned to `2.1.280`
...
The Codex check selects `codex` independently, accepts stable `>=0.147.0 <0.157.0`, and
...
supported range, exact repository qualification version `0.156.1`, closed capabilities,
...
`>=0.147.0 <0.157.0` with exact repository qualification anchor `0.156.1`. Admission and invocation never repeat a lookup or probe. A missing
