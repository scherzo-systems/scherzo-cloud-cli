...
capabilities, and the selected absolute path. Qualification remains pinned to `2.1.263`
...
The Codex check selects `codex` independently, accepts stable `>=0.147.0 <0.155.0`, and
...
supported range, exact repository qualification version `0.154.0`, closed capabilities,
...
`>=0.147.0 <0.155.0` with exact repository qualification anchor `0.154.0`. Admission and invocation never repeat a lookup or probe. A missing
