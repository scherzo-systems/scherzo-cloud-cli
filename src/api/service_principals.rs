...
use crate::service_auth::{ServiceApiKey, ServiceApiKeyError};
...
#[serde(rename_all = "camelCase")]
pub(crate) struct ServicePrincipal {
    pub(crate) id: String,
...
pub(crate) struct IssuedServiceCredential {
    pub(crate) credential: ServiceCredential,
    pub(crate) api_key: Option<ServiceApiKey>,
}
...
#[derive(Debug)]
pub(crate) struct CreatedServicePrincipal {
    pub(crate) principal: ServicePrincipal,
    pub(crate) initial_credential: IssuedServiceCredential,
}

#[derive(Debug, Eq, PartialEq)]
pub(crate) struct ServiceCredentialPage {
    pub(crate) items: Vec<ServiceCredential>,
    pub(crate) next_cursor: Option<String>,
}

#[derive(Debug)]
pub(crate) enum CreateServicePrincipalOutcome {
...
#[derive(Debug, Eq, PartialEq)]
pub(crate) enum ListServiceCredentialsOutcome {
...
#[derive(Debug)]
pub(crate) enum IssueServiceCredentialOutcome {
...
#[derive(Debug, Eq, PartialEq)]
pub(crate) enum RevokeServiceCredentialOutcome {
    Revoked,
    InvalidInput,
    Unauthenticated,
    Forbidden,
...
#[derive(Debug)]
pub(crate) struct ServicePrincipalApiError {
    operation: Operation,
...
enum ServicePrincipalApiErrorKind {
    ...
    InvalidReturnedApiKey(ServiceApiKeyError),
...
impl fmt::Display for ServicePrincipalApiError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match &self.kind {
            ...
            ServicePrincipalApiErrorKind::InvalidReturnedApiKey(_) => write!(
                formatter,
                "the API key in the {} response failed validation",
                self.operation.name()
            ),
...
impl Error for ServicePrincipalApiError {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match &self.kind {
            ...
            ServicePrincipalApiErrorKind::InvalidReturnedApiKey(source) => Some(source),
...
pub(crate) fn create_service_principal(
    client: &HttpClient,
    api_url: &str,
    access_token: &str,
    idempotency_key: &str,
...
pub(crate) fn list_service_credentials(
    client: &HttpClient,
    api_url: &str,
    api_key: &str,
    limit: Option<u16>,
...
pub(crate) fn issue_service_credential(
    client: &HttpClient,
    api_url: &str,
    api_key: &str,
    idempotency_key: &str,
...
pub(crate) fn revoke_service_credential(
    client: &HttpClient,
    api_url: &str,
    api_key: &str,
    credential_id: &str,
...
fn decode_credential_parts(
    ...
    let api_key = api_key
        .map(|value| ServiceApiKey::parse(value.0))
        .transpose()
        .map_err(|source| {
            ServicePrincipalApiError::local(
                operation,
                ServicePrincipalApiErrorKind::InvalidReturnedApiKey(source),
            )
        })?;
    let metadata =
...
fn validate_credential_metadata(
    ...
    if !crate::public_id::valid_typed_id(&id, "crd_") {
        return Err(ServicePrincipalApiError::protocol(
            operation,
...
mod tests {
    ...
    use crate::api::test_support::{REQUEST_IDEMPOTENCY_KEY_ECHO, ScriptedHttpServer};
