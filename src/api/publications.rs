...
fn require_retryable_success_headers(
    ...
    let content_type = headers
        ...
        .transpose()
        .map_err(|()| PublicationFailure::protocol(false))?;
...
fn require_media_type(
    ...
    let actual = response
        ...
        .map(http_util::media_type)
        .transpose()
        .map_err(|()| PublicationFailure::protocol(credential_rejected))?;
...
fn validate_publication(
    ...
    let terminal_at = publication
        ...
        .map(timestamp)
        .transpose()?;
    let identities_valid = crate::public_id::valid_typed_id(&publication.id, "pub_")
        && crate::public_id::valid_typed_id(&publication.organization_id, "org_")
        && crate::public_id::valid_typed_id(&publication.project_id, "prj_")
        && crate::public_id::valid_typed_id(&publication.run_id, "run_")
        && crate::public_id::valid_typed_id(&publication.artifact_set_id, "ats_")
    ...
    let snapshot_valid = publication.run_id == requested_run_id
        ...
        && crate::workflow_contract::is_lowercase_hex(&publication.artifact.base_oid, 40)
        && crate::workflow_contract::is_lowercase_hex(&publication.artifact.head_oid, 40)
        && crate::workflow_contract::is_lowercase_hex(&publication.artifact.tree_oid, 40)
...
fn valid_branch(branch: &models::PublicationBranch, expected_head: &str) -> bool {
    branch.head_oid == expected_head
        && crate::workflow_contract::is_lowercase_hex(&branch.head_oid, 40)
...
#[derive(Debug)]
pub(crate) enum PublicationApiError {
    InvalidEndpoint,
    InsecureHttp,
