...
pub(crate) type RunInputSet = models::RunInputSet;
pub(crate) type RetainedRunInputs = models::RunInputRetainedInventory;

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum NamedInputKind {
    Text,
    Json,
    File,
    Attachments,
...
pub(crate) struct InputScalarMetadata {
    pub(crate) size_bytes: u64,
    pub(crate) sha256: [u8; 32],
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct InputFileMetadata {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct InputAttachmentMetadata {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) enum NamedInputMetadata {
...
impl NamedInputMetadata {
    pub(crate) const fn kind(&self) -> NamedInputKind {
        match self {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct RunInputManifest {
    pub(crate) inputs: BTreeMap<String, NamedInputMetadata>,
}

impl RunInputManifest {
    pub(crate) fn validate(&self) -> Result<(), RunFailure> {
    ...
    pub(crate) fn objects(&self) -> Vec<RunInputObjectMetadata> {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct RunInputObjectMetadata {
    pub(crate) member_id: String,
    pub(crate) attachment_index: Option<i32>,
    pub(crate) display_name: Option<String>,
    pub(crate) media_type: String,
    pub(crate) size_bytes: u64,
    pub(crate) sha256: [u8; 32],
...
pub(crate) struct RunInputUpload<'a> {
    pub(crate) member_id: &'a str,
    pub(crate) bytes: &'a [u8],
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct RunInputUploadOutcome {
    pub(crate) accepted_members: usize,
    pub(crate) verification_required_members: usize,
}

pub(crate) struct RunInputDownloadCapability {
    pub(crate) metadata: RunInputObjectMetadata,
    url: Url,
}

pub(crate) struct ValidatedCapabilityBatch<C> {
...
impl RunApi<'_> {
    ...
    pub(crate) fn upload_input_members(
        &self,
        organization: &str,
        set: &RunInputSet,
    ...
    pub(crate) fn get_retained_inputs(
        &self,
        organization: &str,
        run_id: &str,
    ...
    pub(crate) fn issue_input_download_capabilities(
        &self,
        organization: &str,
        run_id: &str,
    ...
    pub(crate) fn download_input_member(
        &self,
    ...
    pub(crate) fn delete_retained_inputs(
        &self,
        organization: &str,
        run_id: &str,
        idempotency_key: &str,
...
pub(crate) fn capability_batches<T>(members: &[T]) -> impl Iterator<Item = &[T]> {
    members.chunks(CAPABILITY_BATCH_SIZE)
}

pub(crate) fn transfer_capability_batch<T, C, E>(
    batch: &[T],
...
fn validate_input_set(
    ...
    let immutable_valid = crate::public_id::valid_typed_id(&set.id, "ris_")
        ...
        && crate::public_id::valid_typed_id(&set.project_id, "prj_")
...
pub(crate) fn retained_manifest(
    inventory: &RetainedRunInputs,
) -> Result<RunInputManifest, RunFailure> {
    validate_retained_inputs(inventory)
}

pub(crate) fn input_set_is_open(set: &RunInputSet) -> bool {
    set.state == models::run_input_set::State::Open
}

pub(crate) fn input_set_state_name(set: &RunInputSet) -> &'static str {
    match set.state {
...
fn validate_retained_inputs(inventory: &RetainedRunInputs) -> Result<RunInputManifest, RunFailure> {
    ...
    let valid = crate::public_id::valid_typed_id(&inventory.input_set_id, "ris_")
...
fn validated_manifest(wire: &models::RunInputManifestV1) -> Option<RunInputManifest> {
    ...
    for (name, input) in &wire.inputs {
        if !crate::workflow_contract::is_identifier(name) {
            return None;
        }
            ...
            models::RunInputManifestEntry::Attachments(collection) => {
                ...
                for (index, item) in collection.items.iter().enumerate() {
                    if usize::try_from(item.index).ok() != Some(index)
                        || !crate::workflow_contract::is_valid_input_display_name(
...
fn file_metadata(
    ...
    if !crate::workflow_contract::is_valid_media_type(media_type) {
        return None;
    }
...
fn decode_sha256(value: &str) -> Option<[u8; 32]> {
    if !crate::workflow_contract::is_lowercase_hex(value, 64) {
        return None;
    }
