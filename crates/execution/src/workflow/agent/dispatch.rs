...
use crate::workflow::claude_code_stream_json_v1::adapter::ClaudeCodeStreamJsonV1Adapter;
use crate::workflow::codex::CodexConfig;
use crate::workflow::codex_app_server_v1::CodexAppServerV1ProtocolLimits;
use crate::workflow::codex_app_server_v1::adapter::CodexAppServerV1Adapter;
use crate::workflow::diagnostic::StepDiagnosticLog;
use crate::workflow::pi_json_v1::adapter::PiJsonV1Adapter;

pub trait AgentInvocationDispatcher<Sink>: Clone + Send + Sync + 'static
where
...
type ClosedProductionAgentDispatcher<Clock, Observer> = ClosedAgentDispatcher<
...
pub struct ProductionAgentDispatcher<Clock, Observer> {
    inner: ClosedProductionAgentDispatcher<Clock, Observer>,
}
    ...
    Observer: Clone,
{
    ...
    Ok(ProductionAgentDispatcher {
        inner: ClosedAgentDispatcher::new(pi, claude_code, codex),
    })
}

impl<Sink, Clock, Observer> AgentInvocationDispatcher<Sink>
    for ProductionAgentDispatcher<Clock, Observer>
where
    Sink: AgentObservationSink,
    Clock: Clone + Send + Sync + 'static,
    Observer: Clone + Send + Sync + 'static,
    ClosedProductionAgentDispatcher<Clock, Observer>: AgentInvocationDispatcher<Sink>,
{
    fn invoke(
        &self,
        invocation: ClosedAgentInvocation<Sink>,
        ...
        self.inner.invoke(invocation, started, terminal)
    }
}
...
where
    ...
    PiAdapter: AgentAdapter<
            Sink,
            NativeConfiguration = crate::workflow::pi::PiConfig,
            ProtocolLimits = crate::workflow::pi_json_v1::PiJsonV1ProtocolLimits,
        >,
